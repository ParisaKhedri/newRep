...
class Ev3:
    def __init__(self, ip_address):
        ...
        self._s.connect((self._ip_address, self._port))
    ...
    def handle_respons(self):
        ...
        response = self.convert_sensorlist(self.receive_data())
        self._logger.info("received: " + str(response))
        if len(response) != 1:
            raise Ev3Exception("responslist had more the one elements, that should not happen: " + str(response))
        response = response[0]
        if response[0] != "respons":
            raise Ev3Exception("incorrect responsform: " + str(response))
        response = response[1]
        if type(response[0]) != bool:
            raise Ev3Exception("response does not contain a bool telling if the operation was successful or not")
        if not response[0] and type(response[1]) != str:
            print(type(response[1]))
            raise Ev3Exception("response was unsuccessful and does not contain a string telling why")
        if len(response) == 1:
            response.append("")
        if len(response) > 2:
            self._logger.info("response longer than normal")
            temp = []
...
class Motor:
    ...
    def change_attribute(self, att, arg):
        ...
        success, value = self._ev3.handle_respons()
        if success:
            self._ev3._logger.info("successfully changed attribute for motor on port: " + self._port)
        else:
            self._ev3._logger.info("unsuccessfully changed attribute for motor on port: " + self._port)
    ...
    def change_attributes(self, attlist):
        ...
        success, value = self._ev3.handle_respons()
        if success:
            self._ev3._logger.info("successfully changed attributes for motor on port: " + self._port)
        else:
            self._ev3._logger.info("unsuccessfully changed attribute for motor on port: " + self._port)
    ...
    def run_time_limited(self, speed, time_to_run, brake="coast", run=True, finish=False):
        ...
        if self.get_attribute("mode") != "run-timed":
            self.reset()
        attributes = [["mode", "run-timed"]]
        attributes.append(["speed_regulation", True])
        # if self.get_attribute("speed_regulation"):
        speed *= 9
        attributes.append(["speed_sp", speed])
        # else:
        #     attributes.append(["duty_cycle_sp", speed])
        attributes.append(["time_sp", time_to_run])
        attributes.append(["stop_command", brake])
        if run:
            attributes.append(["command", "run-timed"])
        self.change_attributes(attributes)
        if finish and run:
            while "running" in self.get_attribute("state"):
                time.sleep(0.001)
    ...
    def run_position_limited(self, speed, position, brake="coast", run=True, absolute=True, finish=False):
        ...
        attributes = []
        if absolute:
            attributes.append(["mode", "run-to-abs-pos"])
            command = "run-to-abs-pos"
        else:
            attributes.append(["mode", "run-to-rel-pos"])
        ...
        attributes.append(["speed_regulation", True])
        attributes.append(["speed_sp", speed])
        attributes.append(["position_sp", position])
        attributes.append(["stop_command", brake])
        if run:
            attributes.append(["command", command])
        self.change_attributes(attributes)
        if finish and run:
            while "running" in self.get_attribute("state"):
                time.sleep(0.001)
    ...
    def run_forever(self, speed, run=True):
        ...
        if self.get_attribute("mode") != "run-forever":
            self.reset()
        attributes = [["mode", "run-forever"]]
        # if self.get_attribute("speed_regulation"):
        attributes.append(["speed_regulation", True])
        attributes.append(["speed_sp", speed * 9])
        # else:
        #     attributes.append(["duty_cycle_sp", speed])
        if run:
            attributes.append(["command", "run-forever"])
        self.change_attributes(attributes)
